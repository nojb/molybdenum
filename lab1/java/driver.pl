...
my $MAKE_TIMEOUT = 30;		# timeout for making compiler
my $COMPILER = "l1c";		# name of compiler to generate
my $COMPILER_EXEC = "bin/l1c";	# compiler executable
my $REF_COMPILER = "/usr0/autolab/autolab/courses/15411-f09/labs/lab1tests/bin/l1c";
				# reference compiler
my $LEXT = "l1";		# source filename extension
my $GCC = "gcc -m64";		# gcc executable and default flags
my $RT = "l1rt.c";		# runtime system for linking against asm file

my $COMPILER_TIMEOUT = 20;	# timeout for running compiler
...
my $MAX_SCORE0 = 14;		# maximal score for compiler, test suite 0
my $MAX_SCORE1 = 80;		# maximal score for compiler, test suite 1
my $MAX_VALIDATE_SCORE = 6;	# maximal score for test case validation
my $MIN_TESTS = 6;		# minimum number of tests to submit
# my $TESTS1_N = 4;		# first 4 failing suite 1 tests
# my $TESTS1_PTS = 3;		# are worth 3 points each
my $TESTS1_MIN = 122;		# no score below 122
...
if ($validate_tests) {
  ...
  if ($default_tests) {
    ...
    if ($num_tried1) {
      # $score1 = POSIX::floor($num_succeeded1 * $MAX_SCORE1 / $num_tried1);
      # use non-linear scoring for test suite 1
      ...
      #	$score1 = 0;
      #      }
      # use linear score to 122 ($TESTS1_MIN)
