...
sub grade_compiler {
    ...
    if ($Opt_MakeOnly) {
      # if we get here, make must have succeeded
      printf("-- Building compiler succeeded --\n");
      report_result("1", $Opt_Autograding);
      return;
    }
